...
class BlotterController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('admin.blotters.index', compact('blotters', 'stats', 'retentionDays'));
    }
    ...
    public function edit(Blotter $blotter): View
    {
        ...
        return view('admin.blotters.edit', compact('blotter', 'revisions'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $blotter = DB::transaction(function () use ($validated, $request, $complainantName, $remarks) {
            $blotterNumber = Blotter::generateBlotterNumber();

            // Store files in private disk
            ...
            $blotter->blotter_number = $blotterNumber;
    ...
    public function update(Request $request, Blotter $blotter): RedirectResponse
    {
        ...
        if ($changedFields === []) {
            return redirect()
                ->route('admin.blotters.edit', $blotter)
    ...
    // ──────────────────────────────────────────────────
    // DOWNLOAD — Secure, no direct URL exposure
    // ──────────────────────────────────────────────────
    ...
    public function archive(Request $request, Blotter $blotter): RedirectResponse
    {
        ...
        AuditService::log('blotter_archived', $blotter, "Archived blotter {$blotter->blotter_number}");

        return redirect()->route('admin.blotters.index')
            ->with('success', "Blotter {$blotter->blotter_number} archived.");
    }

    // ──────────────────────────────────────────────────
    // RESTORE — Unarchive (admin only)
    // ──────────────────────────────────────────────────
    public function restore(Request $request, int $id): RedirectResponse
    {
        ...
        $blotter->restore();
        $blotter->forceFill(['status' => Blotter::STATUS_ACTIVE])->save();

        AuditService::log('blotter_restored', $blotter, "Restored blotter {$blotter->blotter_number}");

        return redirect()->route('admin.blotters.index')
            ->with('success', "Blotter {$blotter->blotter_number} restored.");
    }
}
