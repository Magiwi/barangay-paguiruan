...
@section('title', 'Appoint Official - e-Governance Admin Panel')

@section('content')
<section class="px-4 py-8 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-2xl">
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-xl font-semibold tracking-tight text-gray-800">Appoint Official</h1>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-6">
            <form method="POST" action="{{ route('admin.officials.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf

                <div>
                    <label for="user_id" class="block text-sm font-medium text-gray-700 mb-1">Staff / Admin User</label>
                    <select name="user_id" id="user_id" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <select name="position_id" id="position_id" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach ($positions as $position)
                            @php $remaining = $position->remainingSeats(); @endphp
                            <option value="{{ $position->id }}" @selected(old('position_id') == $position->id) @disabled($remaining <= 0)>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="term_start" id="term_start" value="{{ old('term_start') }}" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-400">PNG or JPG, max 2MB. Displayed on the Barangay Officials page.</p>
                    @error('photo')
